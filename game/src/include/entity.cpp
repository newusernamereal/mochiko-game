	...
	if (!LOADED_ENTITIES[0].isTrigger) {
		for (int i = 0; i < LOADED_ENTITIES[0].hitboxes.size(); i++) {
			DrawTextureRec(LOADED_ENTITIES[0].hitboxTexts[i],
				{ 0 , 0 , static_cast<float>(LOADED_ENTITIES[0].hitboxes[i].width), static_cast<float>(LOADED_ENTITIES[0].hitboxes[i].height) },
				{ static_cast<float>(LOADED_ENTITIES[0].hitboxes[i].pos.x) , static_cast<float>(LOADED_ENTITIES[0].hitboxes[i].pos.y) },
