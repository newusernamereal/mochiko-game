...
int main(){
	...
	for (int x = 0; x < SIZE; x++) {
		for (int y = 0; y < SIZE; y++) {
			TileGrid[x][y].addBox(EntityHitbox((DoublePoint) { static_cast<double>(x * SCREENX / SIZE) , static_cast<double>(y * SCREENY / SIZE)}, SCREENX / SIZE, SCREENY / SIZE));
	...
	while (!WindowShouldClose()) {
		BeginDrawing();
			if (IsMouseButtonDown(MOUSE_BUTTON_LEFT)) {
				for (int x = 0; x < SIZE; x++) {
					for (int y = 0; y < SIZE; y++) {
						if (TileGrid[x][y].Colliding((DoublePoint) { static_cast<double>(GetMouseX()), static_cast<double>(GetMouseY()) })) {
